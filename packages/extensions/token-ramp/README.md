...
A ramp group may carry its own `$root` token (the anchor swatch as the group's
...
| --- | --- | --- | --- |
| `anchor` | a colour (`"#1DB1A8"`, a stored colour object) or a reference (`"{brand.accent}"`) | yes | The brand colour the ramp is built from. It is never repainted — its step carries it verbatim. A reference resolves in the ramp's own file first, then in the files its compositions resolve it with, then across the collection — so each brand's ramp follows its own anchor even where brands define the same path. |
...
| `ladder`, `lightness`, `lightFraction`, `darkExponent`, `gamut` | as in [settings](#settings) | no | Per-ramp overrides of the configured physics. |
...
| `gamut` | `'srgb'` | The colour space every **computed** stop is mapped into — `'srgb'`, `'display-p3'`, or `'none'` to store the arch's own chroma wherever it lands. Mapping holds lightness and hue and reduces chroma to the gamut boundary, so a stop is always a colour the target can show. The anchor's own step is never mapped. |
