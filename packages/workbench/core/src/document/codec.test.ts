...
const rampish: TokenCodec = {
  key: 'org.test.generate/pair',
  expand(payload, _at, ctx) {
    const p = payload as { anchor?: unknown };
    const anchor = ctx.resolve(p.anchor);
    if (typeof anchor !== 'string') return null;
    return {
      base: { type: 'textCase', value: anchor },
      loud: { type: 'textCase', value: `${anchor}!` },
    };
  },
};

test('a leaf group carrier expands into generated children, anchor resolved through aliases', () => {
  const tokens = parseCollection(
    {
      'core.json': {
        source: { $type: 'textCase', $value: 'uppercase' },
        chain: { $type: 'textCase', $value: '{source}' },
        ramp: {
          $extensions: { 'org.test.generate/pair': { anchor: '{chain}' } },
        },
      },
    },
    [rampish],
  );
  const generated = tokens.filter((t) => t.generated);
  expect(generated.map((t) => t.path.join('.'))).toEqual([
    'ramp.base',
    'ramp.loud',
  ]);
  const base = generated[0] as Token;
  expect(base.value).toBe('uppercase'); // followed the two-link chain
  expect(base.codec).toBe('org.test.generate/pair');
...
test('a group with real children never expands — committed mode leaves the payload inert', () => {
  const tokens = parseCollection(
    {
      'core.json': {
        ramp: {
          $extensions: { 'org.test.generate/pair': { anchor: 'x' } },
          base: { $type: 'textCase', $value: 'committed' },
        },
      },
    },
    [rampish],
  ...
  expect(tokens).toHaveLength(1);
  expect(tokens[0]?.generated).toBeUndefined();
  expect(tokens[0]?.value).toBe('committed');
});

test('an unresolvable anchor leaves the group plain; interchange writes generated stops', () => {
  const dangling = parseCollection(
    {
      'core.json': {
        ramp: {
          $extensions: { 'org.test.generate/pair': { anchor: '{nope}' } },
        },
      },
    },
    [rampish],
  ...
  const files = {
    'core.json': {
      ramp: { $extensions: { 'org.test.generate/pair': { anchor: 'up' } } },
    },
  };
  const tokens = parseCollection(files, [rampish]);
  const staged = interchangeFiles(files, tokens);
  const ramp = (staged['core.json'] as Record<string, unknown>).ramp as Record<
    string,
    unknown
  >;
  expect((ramp.base as Record<string, unknown>).$value).toBe('up');
  expect((ramp.loud as Record<string, unknown>).$value).toBe('up!');
  // The authored file is untouched; the store never holds generated stops.
  expect(
    (files['core.json'].ramp as Record<string, unknown>).base,
  ).toBeUndefined();
});

test('the curation verbs refuse generated tokens, pointing at the payload', async () => {
  const registry = createTokenCodecRegistry();
  registry.register(rampish);
  const document = createDocument({ codecs: () => registry.list() });
  document.hydrate({
    'core.json': {
      ramp: { $extensions: { 'org.test.generate/pair': { anchor: 'up' } } },
  ...
  const { builtinCommands } = await import('../verbs/index');
  const set = builtinCommands().find((c) => c.name === 'token set');
  await expect(async () =>
    set?.run({
      project: { document },
      args: { path: 'ramp.base', value: 'down' },
      options: {},
    }),
  ).rejects.toThrow(/generated by 'org.test.generate\/pair'.*payload/);
});

test('a group carrier may hold $root: the root token parses and the children generate beside it', () => {
  const tokens = parseCollection(
    {
      'core.json': {
        ramp: {
          $root: { $type: 'textCase', $value: 'none' },
          $extensions: { 'org.test.generate/pair': { anchor: 'up' } },
        },
      },
    },
    [rampish],
  ...
  expect(
    tokens.find((t) => t.path.at(-1) === '$root')?.generated,
  ).toBeUndefined();
  // Value codecs still decline a $root-bearing node — it must become a token itself.
  const value = parseCollection(
    {
      'core.json': {
        chip: {
          $root: { $type: 'textCase', $value: 'none' },
          $extensions: { [KEY]: { $type: 'textCase', $value: 'up' } },
        },
      },
    },
    [textCodec],
  );
  expect(value.map((t) => t.path.join('.'))).toEqual(['chip.$root']);
});
